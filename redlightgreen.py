...
# Define the gun sight parameters
sight_size = 30
sight_thickness = 2
sight_color = (0, 255, 0)  # Green color for the gun sight
...
while True:
    ...
    if inFrame == 1:
            ...
            if cPos >= 100:
                ...
                if (cEnd - cStart) <= 3:
                    ...
                    if abs(tempSum - userSum) > 150:
                        ...
                        isAlive = 0
                        # Draw the gun sight on the frame
                        center_x = int(res.pose_landmarks.landmark[0].x * frm.shape[1])
                        center_y = int(res.pose_landmarks.landmark[0].y * frm.shape[0])
                        cv2.line(frm, (center_x - sight_size, center_y), (center_x + sight_size, center_y), sight_color,
                                 sight_thickness)
                        cv2.line(frm, (center_x, center_y - sight_size), (center_x, center_y + sight_size), sight_color,
                                 sight_thickness)
        ...
        cv2.putText(frm, "", (20, 200), cv2.FONT_HERSHEY_SIMPLEX, 0.8,
                    (0, 255, 0), 4)
